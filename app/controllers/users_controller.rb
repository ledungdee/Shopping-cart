...
class UsersController < ApplicationController
  before_action :logged_in_user, only: %i[edit update index destroy show]
  ...
  def destroy
    ...
    if @user.shop.present?
      @user.shop.products.destroy_all if @user.shop.products.present?
      @user.shop.destroy
    end
    @user.destroy
  ...
  def user_params
    params.require(:user).permit(:name, :email, :password, :password_confirmation, :phone_number, :address,
                                 :delivery_address, :image, :image_cache)
  end
  ...
  def correct_user
    ...
    return unless current_user?(@user) == false
